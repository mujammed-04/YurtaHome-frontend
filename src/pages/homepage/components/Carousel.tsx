...
const Carousel: React.FC<Props> = ({
  ...
  return (
    <div className="carousel">
      ...
      >
        {interior &&
          items.map((item) => <InteriorItem interior={item} key={item.id} />)}

        {house && items.map((item) => <RoomItem room={item} key={item.id} />)}

        {discount &&
          items.map((item) => <DiscountItem discount={item} key={item.id} />)}
      </div>
    </div>
  );
};
